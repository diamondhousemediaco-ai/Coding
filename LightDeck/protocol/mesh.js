  ...
  class Node {
    ...
    async _onNet(type, data) {
      ...
      try {
        ...
        let upper, akf, aszmic = 0, seqForNonce = d.seq, src = d.src, dst = d.dst;
        if (t[0] & 0x80) {                                     // segmented access — reassemble
          akf = (t[0] >> 6) & 1; aszmic = (t[1] >> 7) & 1;
          const seqZero = ((t[1] & 0x3f) << 6) | (t[2] >> 2);
          const segO = ((t[2] & 0x03) << 3) | (t[3] >> 5), segN = t[3] & 0x1f;
          const kk = src + ':' + seqZero; this._segbuf = this._segbuf || {};
          const sb = this._segbuf[kk] || (this._segbuf[kk] = { parts: [], segN });
          sb.parts[segO] = t.slice(4);
          if (segO === 0) { let s = (d.seq & ~0x1fff) | seqZero; if (s > d.seq) s -= 0x2000; sb.seqAuth = s; }
          let have = 0; for (let i = 0; i <= segN; i++) if (sb.parts[i]) have++;
          if (have <= segN) return;                            // still waiting for segments
          delete this._segbuf[kk]; upper = cat(...sb.parts); seqForNonce = sb.seqAuth != null ? sb.seqAuth : d.seq;
        } else { akf = (t[0] >> 6) & 1; upper = t.slice(1); }
        const key = akf ? this.appKey : this.devKey;
        const dec = await upperDecrypt(key, { devKey: !akf, seq: seqForNonce, src, dst, ivIndex: this.net.ivIndex, aszmic }, upper);
        if (!dec.ok) { if (this._log) this._log('← 0x' + src.toString(16) + ' reply (could not decrypt)'); return; }
        ...
        if (this._waiters && this._waiters[op]) { const w = this._waiters[op]; delete this._waiters[op]; w(params); }
    ...
    _await(op, ms = 5000) { this._waiters = this._waiters || {}; return new Promise(res => { this._waiters[op] = res; setTimeout(() => { if (this._waiters[op] === res) { delete this._waiters[op]; res(null); } }, ms); }); }
    _seq() { const s = this.seq | 0; this.seq = s + 1; if (this.onSeq) this.onSeq(this.seq); return s; }
    async _sendAccess(o, ap) { const seq = this._seq(); const pdus = await encodeAccessMessages(this.net, { ...o, seq }, ap); if (pdus.length > 1) this.seq += pdus.length - 1; for (const p of pdus) { try { await this.bearer.send(0x00, p.slice(1)); } catch (e) { await sleep(80); await this.bearer.send(0x00, p.slice(1)); } await sleep(40); } }
    // Ask the light for its element/model map (Config Composition Data Get, page 0).
    async getComposition(log = () => {}) {
      const wait = this._await(0x02, 6000); log('Config: Composition Data Get…');
      await this._sendAccess({ devKey: this.devKey, src: this.provisionerAddr || 0x0001, dst: this.unicast }, cat(h2b('8008'), h2b('00')));
      const params = await wait; if (!params) { log('✗ no Composition Data Status (timed out)'); return null; }
      let i = 1; const rd = () => { const v = params[i] | (params[i + 1] << 8); i += 2; return v; };
      const cid = rd(), pid = rd(), vid = rd(), crpl = rd(), feat = rd(); const elements = []; let addr = this.unicast;
    ...
    async configure(models = [0x1000, 0x1300, 0x1303, 0x1306, 0x1307], log = () => {}) {
      const src = this.provisionerAddr || 0x0001, dst = this.unicast;
      const comp = await this.getComposition(log);
      log('Config: AppKey Add…'); await this._sendAccess({ devKey: this.devKey, src, dst }, cfg.appKeyAdd(0, 0, this.appKey)); await sleep(500);
      this.routes = {};
      if (comp) {
        for (const el of comp.elements) for (const m of el.sig) if (models.includes(m)) {
          log('Config: bind 0x' + m.toString(16) + ' @ elem 0x' + el.addr.toString(16) + '…');
          await this._sendAccess({ devKey: this.devKey, src, dst }, cfg.modelAppBind(el.addr, 0, m)); await sleep(350);
          if (this.routes[m] == null) this.routes[m] = el.addr;
        }
        for (const el of comp.elements) if (el.ven.length) log('  vendor model(s) on elem 0x' + el.addr.toString(16) + ': ' + el.ven.map(x => '0x' + x.toString(16)).join(',') + ' (effects/CCT may need these)');
      ...
      log('Config done. Routes: ' + Object.keys(this.routes).map(m => '0x' + (+m).toString(16) + '→0x' + this.routes[m].toString(16)).join(', '));
      return this.routes;
    }
    ...
    lightness(v) { return this._sendAccess({ appKey: this.appKey, src: this.provisionerAddr || 0x0001, dst: this._route([0x1300, 0x1303]) }, accLightness(v)); }
    ctl(l, temp, duv = 0) { return this._sendAccess({ appKey: this.appKey, src: this.provisionerAddr || 0x0001, dst: this._route([0x1303, 0x1306]) }, accCTL(l, temp, duv)); }
    hsl(l, hue, sat) { return this._sendAccess({ appKey: this.appKey, src: this.provisionerAddr || 0x0001, dst: this._route([0x1307]) }, accHSL(l, hue, sat)); }
  }
